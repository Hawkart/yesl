...
class SteamHelper{
    ...
    {        
        ...
        {
            foreach(array_chunk($steam_ids, 100) as $ids)
            {
    ...
    public static function getDota2PlayerMatches($steam_id32, $params = [])
    {
        if(strlen((string)$steam_id32)>=16)
            $steam_id32 = self::convertSteamid64ToSteamid32($steam_id32);
        
        $openDota = new OpenDotaHelper(['returnJson' => true], []);
        return $openDota->getPlayerMatches($steam_id32, $params);
    } 
