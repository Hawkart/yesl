...
class ImportCollegeScoreCard extends Command
{
    ...
    private function addUniversity($university)
    {
        ...
        {
            if($university['school.operating']==1 && in_array($university['school.degrees_awarded.highest'], [3,4]) && $university['school.online_only']==0)
            {
                $url = $university['school.school_url'];
                $domain = str_ireplace('www.', '', parse_url($url, PHP_URL_HOST));

                University::create([
                    'nace' => in_array(strtoupper($university['school.name']), $this->naces),
